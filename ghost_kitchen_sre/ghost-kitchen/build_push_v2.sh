...
# Build RAG Service (The Knowledge Base / Token Factory)
echo "--- Building RAG Service ---"
docker build -t "$DOCKER_USER/$DOCKER_REPO:rag-$VERSION" -f Dockerfile.rag .
...
# Included 'rag' in the service loop
services=("agent" "dashboard" "producer" "flink" "rag")
