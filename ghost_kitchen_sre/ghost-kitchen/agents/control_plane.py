...
def get_llm():
    return OllamaLLM(model="llama3.1:8b", base_url="http://ollama:11434", temperature=0.1)
...
def context_fetcher(state: ControlState) -> ControlState:
    ...
    except: state["k8s_events"] = ["No K8s events found."]
    return state
...
def policy_engine(state: ControlState) -> ControlState:
    ...
    Decide: SCALE, RESTART, or KILL_POD. Return ONE word."""
    try:
    ...
    except: state["policy_decision"] = "RESTART"
    return state
...
def executor(state: ControlState) -> ControlState:
    ...
    except Exception as e: state["execution_status"] = f"Error: {str(e)}"
    return state

workflow = StateGraph(ControlState)
workflow.add_node("context", context_fetcher); workflow.add_node("policy", policy_engine); workflow.add_node("exec", executor)
workflow.add_edge("context", "policy"); workflow.add_edge("policy", "exec"); workflow.add_edge("exec", END)
workflow.set_entry_point("context")
agent_app = workflow.compile()

def main():
    bootstrap = os.getenv("BOOTSTRAP_SERVERS", "redpanda-0.redpanda.kitchen-sre.svc.cluster.local:9092")
    consumer = KafkaConsumer('t1_kitchen.alerts', bootstrap_servers=bootstrap, value_deserializer=lambda x: json.loads(x.decode('utf-8')))
    producer = KafkaProducer(bootstrap_servers=bootstrap, value_serializer=lambda v: json.dumps(v).encode('utf-8'))

    for msg in consumer:
        res = agent_app.invoke({"alert": msg.value, "slo_metrics": msg.value})
        producer.send('t1_kitchen.actions', {**msg.value, "policy": res["policy_decision"], "status": res["execution_status"]})
